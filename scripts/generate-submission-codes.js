...
const dedupeRegistrationsByEmail = (registrations = []) => {
  const seen = new Map();
  const unique = [];
  const duplicates = [];
  ...
  for (const registration of registrations) {
    ...
    if (seen.has(email)) {
      duplicates.push({
        email,
        duplicateTeam: registration.teamName,
        originalTeam: seen.get(email)?.teamName || 'Unknown',
      });
      continue;
    }

    seen.set(email, registration);
    unique.push(registration);
  }
...
async function generateSubmissionCodes() {
  ...
  try {
    ...
    const { unique: uniqueRegistrations, duplicates } = dedupeRegistrationsByEmail(registrations);

    console.log(
      `✓ Found ${registrations.length} team registrations (processing ${uniqueRegistrations.length} unique lead emails)`
    );

    if (duplicates.length) {
      console.log('⚠️  Duplicate lead emails detected (keeping earliest registration per email):');
      for (const duplicate of duplicates) {
        console.log(
          `   • ${duplicate.email} → keeping "${duplicate.originalTeam}"; skipping "${duplicate.duplicateTeam}"`
        );
      }
      console.log('');
    } else {
    ...
    for (const registration of uniqueRegistrations) {
      ...
      const existingPlainCode = plainCodes[leadEmail];
      const hasPlainCode = Boolean(existingPlainCode);
      
      if (existing && hasPlainCode && !updateExisting) {
        // Keep existing code (we have both hash and plain code)
        newAccessCodes.push(existing);
        newPlainCodes[leadEmail] = existingPlainCode;
        
        existingCount++;
        console.log(`  ↻ ${registration.teamName} (${leadEmail}) - Using existing code`);
      } else {
        // Generate new code (either no existing, or existing but no plain code, or updateExisting flag)
        ...
        newCount++;
        if (existing && !hasPlainCode && !updateExisting) {
          console.log(`  ✓ ${registration.teamName} (${leadEmail}) - ${accessCode} (regenerated plain code)`);
        } else if (existing && updateExisting) {
          console.log(`  ✓ ${registration.teamName} (${leadEmail}) - ${accessCode} (regenerated by request)`);
        } else {
